...
> Complete reference for the datamitsu configuration API
...
## Managed Configs (`managedConfigs`)

Managed config entries define project-owned files that `datamitsu config reconcile`
writes before running `datamitsu fix`. Use `datamitsu config reconcile --dry-run`
to preview without either action.
...
interface ManagedConfig {
  ...
  tools?: string[]; // Tool name(s) this config belongs to; enables `config reconcile --tools` scoping
}
```

The optional `tools` field associates a config file with one or more tools (by
name, matching keys in [`tools`](#tools-tools)). `datamitsu config reconcile --tools <names>`
...
plain `datamitsu config reconcile` would re-derive the file and could silently drop your
overrides.
...
On every `datamitsu config reconcile` dry-run or normal run, the hash is
recomputed and compared. When it matches, reconciliation proceeds normally.
When the upstream chain has **drifted**, reconciliation aborts **before writing anything** and prints the new hash
...
const managedConfigs = {
  "eslint.config.js": {
    projectTypes: ["typescript", "javascript"],
    tools: ["eslint"], // scope `config reconcile --tools eslint` to this file
    content: (context) => {
