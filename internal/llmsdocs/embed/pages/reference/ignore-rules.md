...
them one at a time. Run [`datamitsu setup`](./cli-commands.md#setup) with the
`--opt-in-tools` flag:
...
This runs the normal setup and then writes a `.datamitsuignore` at your git root
that disables all configured tools via a single catch-all rule:
...
`datamitsu fix`. Setup refuses to overwrite an existing `.datamitsuignore` so your
manual selections are never lost; pass `--dry-run` to preview the file without
writing it.
