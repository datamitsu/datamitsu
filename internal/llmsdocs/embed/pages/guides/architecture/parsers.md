...
> How datamitsu declares, builds, signs, delivers, downloads, and loads sandboxed Rust→WASM output parsers, and how the line-based diff-in-core powers formatting
...
```mermaid
graph LR
    A["Declare(parsers entry)"] --> B["Build(Rust → WASM, CI)"]
    B --> C["Sign(checksums.txt + cosign)"]
    C --> D["Deliver(GitHub Release asset)"]
...
```mermaid
graph LR
    R["parsers entryurl + SHA-256"] --> D["Download (retry)"]
    D --> V{"SHA-256matches?"}
