...
The repository root is a pure function of the working directory within one process — a command never changes directory mid-run, and the long-lived `datamitsu lsp` changes it only to find the root it serves and to load the config there, one step at a time, never while another lookup or load is running. It is therefore resolved **once per process** for each working directory and memoized for the process lifetime, keyed by the directory the lookup started from. Overlapping callers collapse onto a single resolution rather than racing to duplicate it, and failures are cached too, because a directory does not become a repository mid-run.
...
Two helpers resolve the root today. `facts.GetGitRoot` is the memoized one described here, used by the config loader, by every engine construction, and by `lsp`, which resolves the root it serves with the same helper as the config it loads there. `traverser.GetGitRoot` takes an explicit directory and is used by command handlers (`exec`, `init`, `config reconcile`, `cache`) and the runner; it is not memoized, has no pure-Go path, and ignores `DATAMITSU_FORCE_GIT_SUBPROCESS`. Consolidating the two is follow-up work.
