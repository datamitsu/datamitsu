...
flowchart LR
    subgraph Producer["Producer (CI, community toolchain)"]
        DF["devtools dockerfile--emit-oci-map"] --> BX["docker buildx(per-subtree COPY --link)"]
        BX --> PP["post-process(regctl: annotations, bundle index)"]
        PP --> REG[("OCI registryghcr.io/owner/tool-store")]
    end
    subgraph Consumer["Consumer (no docker)"]
        REG -->|"pull by digest,sha256-verified"| SEED["datamitsu store seed /auto-seed"]
