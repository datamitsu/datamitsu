...
#### Re-harvesting the embedded documentation

These same pages ship inside the binary, so that `datamitsu llms` can hand an AI
agent documentation that matches its exact version without network access. The
snapshot lives in `internal/llmsdocs/embed/` and is committed to git, which is
what keeps a plain `go build` free of any website toolchain.

After changing anything under `website/docs`, regenerate it:
...
That rebuilds the site, harvests the cleaned Markdown that
`docusaurus-plugin-llms` emits, and rewrites the snapshot. CI re-runs the same
harvest on every pull request and fails if the result differs from what you
committed, so a stale snapshot cannot merge.

Two rules matter when touching the snapshot:

- **Never edit `internal/llmsdocs/embed/` by hand.** It is generated. The page
  bytes are hashed into `manifest.json`, and the tests verify that hash.
- **Never reformat it.** The pages are stored exactly as the plugin emitted
  them; `.datamitsuignore` excludes the directory from every tool for this
  reason.
