...
- `filterValidAssets()` in `detect.go` pre-filters assets before scoring: rejects checksum files (`IsChecksumFile` — `.sha256`, `.md5`, etc.) and non-executable package formats (`IsNonExecutableFile` — `.vsix`, `.deb`, `.rpm`, `.nupkg`, `.whl`); prevents package-manager bundles from outscoring real binaries via neutral libc penalty
