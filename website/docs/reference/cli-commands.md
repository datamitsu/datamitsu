...
Before scoring, the detector filters out two categories of assets: checksum files (`.sha256`, `.md5`, `.sha512`, etc.) and non-executable package formats (`.vsix`, `.deb`, `.rpm`, `.nupkg`, `.whl`). This prevents IDE extensions or package-manager bundles from outscoring actual binaries when releases mix executable and non-executable assets.
