---
title: Source Mode
description: Put a project's declared toolchain on PATH for the current shell, so every tool runs at the version the repository pins — with lazy downloads and invisible branch switching
---
...
```mermaid
flowchart TD
    CFG["datamitsu.config.ts<br/>+ wrapper configs"] --> RES["Resolve every declared app<br/>(no downloads, no network)"]
    RES --> PLAN["Plan<br/>entries · exclusions · shadows"]
    PLAN --> FARM["Farm directory<br/>{cache}/projects/{hash}/bin"]
    PLAN --> MAN["manifest.json<br/>watch set + staleness key"]
    FARM --> ACT["Activation<br/>farm first on PATH"]
    ACT --> RUN["tofu plan"]
    RUN --> DISP["argv[0] dispatch<br/>read manifest, check freshness"]
