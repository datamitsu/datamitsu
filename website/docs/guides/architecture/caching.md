...
```mermaid
graph TD
    P["Pre-run: hash unit inputs"] -->|"reads through memo"| M[("Content-hash memo<br/>path → hash, size, mtime")]
    P --> R["Run tool"]
    R --> Q{"Operation?"}
    Q -->|"lint (read-only)"| S["Re-stat every path,<br/>re-hash only what moved"]
