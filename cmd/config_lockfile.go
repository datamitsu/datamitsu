...
var configLockfileCmd = &cobra.Command{
	...
	Long: `Reinstalls a runtime-managed app (fnm/node/uv/go) and outputs its lock file content
as a JSON-escaped string ready to paste into configuration.

When called without arguments, lists all apps that support lock files (fnm/node/uv/go).
...
func runConfigLockfile(cmd *cobra.Command, args []string) error {
	...
	if app.Fnm == nil && app.Node == nil && app.Uv == nil && app.Go == nil {
...
// FNM/Node/UV/Go LockFile field cleared. The original map and runtime configs are not
...
func printAppInfo(appName string, app binmanager.App) {
	...
	if app.Fnm != nil {
		...
		fmt.Fprintf(os.Stderr, "  Version:      %s\n", app.Node.Version)
		if len(app.Node.Dependencies) > 0 {
			fmt.Fprintf(os.Stderr, "  Dependencies:\n")
			keys := make([]string, 0, len(app.Node.Dependencies))
			for k := range app.Node.Dependencies {
				keys = append(keys, k)
			}
			sort.Strings(keys)
			for _, k := range keys {
...
func listLockfileApps(apps binmanager.MapOfApps) {
	var fnmApps, nodeApps, uvApps, goApps []string
	...
	if len(fnmApps) == 0 && len(nodeApps) == 0 && len(uvApps) == 0 && len(goApps) == 0 {
