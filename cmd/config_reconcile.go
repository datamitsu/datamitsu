...
var (
	...
	reconcileSkipFix       bool
	reconcileOptInTools    bool
	reconcileSelectedTools string
	reconcileNoVerifyHash  bool
)

// optInIgnoreFilename is the name of the ignore file generated by
// `datamitsu config reconcile --opt-in-tools`, written at the git root.
const optInIgnoreFilename = ".datamitsuignore"

var configReconcileCmd = &cobra.Command{
	Use:   "reconcile",
	Short: "Reconcile managed configuration files",
	Long: `Reconcile managedConfigs entries by creating, replacing, patching, linking,
or removing project-owned files for the detected project types. The command
...
func init() {
	configReconcileCmd.Flags().BoolVar(&reconcileDryRun, "dry-run", false, "Show what would be reconciled without writing files or running fix")
	configReconcileCmd.Flags().BoolVar(&reconcileSkipFix, "skip-fix", false, "Skip running fix after reconciling managed configuration files")
	configReconcileCmd.Flags().BoolVar(&reconcileOptInTools, "opt-in-tools", false, "Include an all-disabled .datamitsuignore in reconciliation so tools are opt-in (enable them by removing names from the list)")
	configReconcileCmd.Flags().StringVar(&reconcileSelectedTools, "tools", "", "Comma-separated list of tools to scope reconciliation to (only their managed config files are considered)")
	configReconcileCmd.Flags().BoolVar(&reconcileNoVerifyHash, "no-verify-hash", false, "Skip expectChainHash verification even when a pinned managed config drifted from its upstream chain")
	configCmd.AddCommand(configReconcileCmd)
}

func runConfigReconcile(_ *cobra.Command, _ []string) error {
	...
	// When generating an opt-in ignore file, fail fast before reconciliation if a
	// .datamitsuignore already exists so manual tool selections are never lost.
	if reconcileOptInTools {
		if err := ensureNoExistingIgnore(rootPath); err != nil {
			return err
		}
	}

	cfg, layerMap, vm, err := loadConfigForReconcile(ctx)
	if err != nil {
	...
	// overwritten. Runs during dry-runs too. Printed before the display is activated.
	if err := verifyChainHashes(layerMap, reconcileNoVerifyHash); err != nil {
		return err
	}

	// Parse and validate --tools: scopes which managed config files reconciliation considers.
	selectedTools := parseSelectedTools(reconcileSelectedTools)
	...
	// Repo-wide, git-root-relative project locations exposed to managed config content()
	...
	// Activate the shared display and frame the whole run as one reconciliation bracket.
	...
	disp.PhaseOpen("config reconcile")
	if detectedCount == 0 {
	...
	for _, result := range results {
		...
		case "created":
			created = append(created, reconcileRow{clr.Green("+"), relPath})
			fileCount++
		case "patched":
			patched = append(patched, reconcileRow{clr.Faint("~"), relPath})
			fileCount++
		case "replaced":
		...
		case "linked":
			...
			linked = append(linked, reconcileRow{clr.Cyan("↳"), text})
			fileCount++
		}
		for _, deleted := range result.DeletedFiles {
			...
			removed = append(removed, reconcileRow{clr.Red("−"), relDeleted})
		}
		if result.Error != nil {
			failedRows = append(failedRows, reconcileRow{clr.Red("✗"), relPath + clr.Faint(": "+result.Error.Error())})
			installErrors = append(installErrors, fmt.Errorf("%s: %w", result.ConfigName, result.Error))
		}
	}

	reconcileSection(disp, "created", created)
	...
	reconcileSection(disp, "errors", failedRows)

	// Surface tool-scoped no-ops (a tool may own no config, or its only config
	// may not apply here). Informational, not an error.
	for _, t := range toolsWithoutGeneratedConfig(selectedTools, cfg.ManagedConfigs, results) {
		disp.PhaseBody(clr.Faint("no config generated for " + t))
	}

	// Generate the all-disabled opt-in ignore file before the post-reconciliation
	// fix so that, when it runs, it already respects the opt-in state.
	if len(installErrors) == 0 && reconcileOptInTools {
		if err := writeOptInIgnore(rootPath, cfg.Tools, dryRun); err != nil {
			return err
		}
	}

	printReconcileFooter(disp, reconcileFooterCounts{
		files:    fileCount,
		created:  len(created),
	...
	// renders its own frame (it reuses the banner already shown above).
	closeReconcile()

	if !dryRun && !reconcileSkipFix {
		if err := runner.RunContinuation(config.OpFix, []string{}, "", false, reconcileSelectedTools, func() (*config.Config, string, error) {
...
// reconcileRow is one item line in a reconciliation action group.
type reconcileRow struct {
	sym  string // colored glyph
	text string
}

// reconcileSection renders one "┃ <title>" group with "┃   <sym> <text>" item rows.
// Nothing is printed for an empty group.
func reconcileSection(disp *ui.Display, title string, rows []reconcileRow) {
	if len(rows) == 0 {
		return
	}
	disp.PhaseBody("")
...
// reconcileFooterCounts carries the tallies shown in the closing bracket rule.
type reconcileFooterCounts struct {
	files, created, replaced, linked, patched, removed, failed int
	dur                                                        string
	dryRun                                                     bool
}

// printReconcileFooter renders the closing "┗━ … ━" rule summarizing the run.
func printReconcileFooter(disp *ui.Display, c reconcileFooterCounts) {
...
// which reconciliation produced no config file — either because no config is associated
...
func toolsWithoutGeneratedConfig(selected []string, configs config.MapOfManagedConfigs, results []install.InstallResult) []string {
	if len(selected) == 0 {
		return nil
	}
...
func buildOptInIgnoreContent(tools config.MapOfTools) (string, int) {
	...
	b.WriteString("# Generated by `datamitsu config reconcile --opt-in-tools`.\n")
...
// filesystem; otherwise it writes the file with 0o644 permissions.
