...
func TestConfigReconcileCommandRegistered(t *testing.T) {
	found := false
	...
	if !found {
		t.Error("reconcile command not registered with configCmd")
	}

	for _, cmd := range rootCmd.Commands() {
		if cmd.Name() == "setup" {
			t.Fatal("removed setup command is still registered with rootCmd")
		}
	}
}

func TestConfigReconcileCommandFlags(t *testing.T) {
	flags := configReconcileCmd.Flags()
	...
	for _, tt := range tests {
		...
		if f == nil {
			t.Errorf("flag %q not found on config reconcile command", tt.name)
			continue
		}
	...
	for _, name := range []string{"apply", "fix"} {
		if flags.Lookup(name) != nil {
			t.Errorf("removed flag %q is still registered on config reconcile command", name)
		}
	}
}
...
func TestConfigReconcilePassesLayerMapToNewInstaller(t *testing.T) {
	...
	layerMap := &config.ManagedConfigLayerMap{
		".editorconfig": &config.ManagedConfigLayerHistory{
...
func TestDryRunModeLayerHistoryStillBuilt(t *testing.T) {
	...
	// dry-run mode is checked. This test verifies the layerMap is always populated
	...
	layerMap := &config.ManagedConfigLayerMap{
		".editorconfig": &config.ManagedConfigLayerHistory{
			...
			FinalConfig: config.ManagedConfig{Scope: config.ScopeGitRoot},
		},
	}

	configs := config.MapOfManagedConfigs{
		".editorconfig": config.ManagedConfig{Scope: config.ScopeGitRoot},
	}
