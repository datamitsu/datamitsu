...
//! through `from_json`; here we map them directly via overridden `Attrs`. The
//! report is written to the `--report-path -` target which the builtin reads
//! from stderr (`from_stderr = true`), so we parse stderr. Source is the static
//! label "gitleaks"; gitleaks emits no severity token.
