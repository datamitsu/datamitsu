...
- [x] update `README.md` / `AGENTS.md` / `CLAUDE.md` and any runtime docs describing the Node runtime: now a direct archive (url+hash) like jvm; remove fnm mentions; note `fnmApps.json` → `nodeApps.json` — README already minimal (no fnm); `AGENTS.md` (== `CLAUDE.md` symlink) fnm refs updated (lock-files line, pnpm-defaults rationale `fnm.go`→`pnpm.go`, wrapper-maintenance + npm-Tabs notes → node). Swept all of `website/docs/**` (18 files), `docs/architecture.md`, and `website/src/pages/index.tsx`: every `fnm` runtime kind, `fnm:` app sub-object, FNM-Node/PNPM prose, cache paths (`.apps/fnm`→`.apps/node`, `.runtimes/fnm-nodes`→`.runtimes/node`, `.runtimes/fnm-pnpm`→`.runtimes/pnpm`), Schniz/fnm release-zip examples (→ nodejs.org/unofficial-builds `.tar.xz`/`.zip` archive entries), and the obsolete `FNM_NODE_DIST_MIRROR`/`FNM_ARCH` musl-mirror mechanism (use-in-alpine.md fully rewritten to static musl registry entries) replaced. Cross-file anchors kept consistent (`#pnpm-node-apps`, `#node-apps-npm-devtools-pull-node`, `#nodejs-musl-static-archive-entries`). Repo-wide `rg -i fnm` over the docs surface returns zero (only the intentional migration page mentions fnm). `datamitsu check` green (16 tools)
- [x] add a CHANGELOG / migration note: `fnm` runtime kind removed (clean break) — consumers must re-init; node now pinned by sha256 — new `website/docs/how-to/migrate-fnm-to-node.md` (registered in `website/sidebars.ts`): `:::warning` clean-break note + re-init command, before/after table (kind, acquisition, hash pinning, musl, `fnm:`→`node:`, `fnmApps.json`→`nodeApps.json`, `pull-fnm`→`pull-node`), step-by-step config migration, cache-layout rename table, and Alpine/musl section
- [x] document the `devtools pull-node` workflow (how to bump the node version + refresh hashes, incl. glibc `.asc` provenance check) — added a "Bumping the Node.js runtime" subsection to `website/docs/how-to/maintain-wrapper.md` covering `pull-runtimes --update --runtime node`: latest-LTS resolution, per os/arch/libc URL + SHA-256 generation, glibc/darwin/windows hashes GPG-verified against nodejs.org's signed `SHASUMS256.txt.asc` (28 embedded Node release keys, invalid signature aborts), musl from unofficial-builds.nodejs.org's unsigned `SHASUMS256.txt` (git-pinned, `node:alpine` trust model), plus post-bump lockfile regeneration + commit. `cli-commands.md` + `maintain-wrapper.md` `pull-fnm`/`pull-node` (apps) reference also updated
