...
- [x] Add a runtime wrapper (in `internal/parsermanager` or `internal/parserwasm`): instantiate
      ...
      (`internal/parsermanager/runtime.go`: `ParserRuntime` (NewRuntime/Parse/Close) drives
      alloc→write→parse→read→dealloc; decodes into `RawDiagnostic` — pointer fields so a
      JSON-absent field stays nil per the nullable contract. Replaced the `deps.go` blank
      wazero import with the real one.)
- [x] Commit a small prebuilt `echo.wasm` test fixture under `testdata/` (regenerated by the
      `build:parsers` task), so the Go test does not require cargo at `go test` time.
      (`internal/parsermanager/testdata/echo.wasm`, 27830 bytes — the real Rust→WASM module.)
- [x] Write tests: load `echo.wasm`, invoke with sample input, assert the fixed echo output;
      malformed module → error. (`runtime_test.go`: echo round-trip, multiline+exit-code
      preserved, unknown-tool → empty, malformed module → instantiate error, plus an
      end-to-end `ParseOutput` test through the Manager.)
- [x] **Dog-food 3:** on this repo, the core downloads the echo parser into the store, loads it
      ...
      (Covered in test form: `TestParseOutput_EndToEnd` downloads via httptest → store → wazero →
      fixed echo answer and asserts the second invocation reuses the store (1 hit); corrupt-hash
      failure is `TestLoadWASMBytes_WrongHashFails`. `golangci-lint` clean; CLI golden byte-stable.)
