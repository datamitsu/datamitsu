...
- **pnpm-workspace.yaml auto-merge** (FNM apps, `fnm.go`): before `pnpm install`, the installer always writes `pnpm-workspace.yaml` to the app environment via `writeFNMAppWorkspaceFile()`. Content is built by `buildPNPMWorkspaceForApp(files)` which starts from `defaultPNPMWorkspaceConfig()` (hardcoded pnpm 11 security defaults: `strictDepBuilds: true`, `blockExoticSubdeps: true`, `enablePrePostScripts: false`, `dangerouslyAllowAllBuilds: false`, `minimumReleaseAge: 10080`, `trustPolicy: "no-downgrade"`, `lockfile: true`, `preferFrozenLockfile: true`) and shallow-merges the user's `App.files["pnpm-workspace.yaml"]` entry on top (user wins per top-level key). The entry is consumed by the merge — `writeFNMAppWorkspaceFile()` returns a filtered files map (without `pnpm-workspace.yaml`) for `WriteAppFiles()` to avoid double-writing. Defaults are also written when no user entry is present (backward-compatible)
...
- Well-known keys published by the embedded default config (`internal/config/config.js`):
  ...
  - `"pnpm-workspace-defaults"` — YAML string of pnpm 11 security defaults (same set baked into `defaultPNPMWorkspaceConfig()` in `internal/runtimemanager/fnm.go`); intended for downstream configs that want to write a hardened `pnpm-workspace.yaml` into a project repo (e.g., via a Bundle) — distinct from the auto-merge applied to per-FNM-app `files["pnpm-workspace.yaml"]`
