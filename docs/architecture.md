...
- `devtools` - Developer utility commands for config maintenance (see [cmd/devtools.go](cmd/devtools.go))
  ...
  - `devtools pull-runtimes <file>` - Pull runtime configurations (Node, UV, JVM) from upstream releases (see [cmd/devtools_pull_runtimes.go](cmd/devtools_pull_runtimes.go)); requires file argument (path to runtimes.json); fetches latest releases from upstream, computes SHA-256 hashes, writes to `<file>`; supports `--update` (required), `--dry-run`, `--runtime <name>` (uv, jvm, or node); version sources: Node.js LTS from endoflife.date, pnpm from npm registry, Python stable from endoflife.date, Java from Adoptium API. The node entry resolves the latest Node LTS, builds os/arch/libc archive URLs and records SHA-256: glibc/darwin/windows hashes are GPG-verified against nodejs.org's clearsigned `SHASUMS256.txt.asc` (28 embedded official Node.js release keys, `internal/nodekeys` package), while musl hashes come from unofficial-builds.nodejs.org's unsigned `SHASUMS256.txt` (git-pinned, node:alpine trust model); deduplicates musl binaries identical to glibc
